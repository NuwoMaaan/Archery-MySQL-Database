...
func GetEndCountOrderTotalByDate(db *sql.DB) (models.QueryResult, error) {
	query := "SELECT RoundNum, round.RoundName, archer.ArcherID, archer.FirstName, " +
		"Date, archer.LastName, SUM(total) AS RoundTotalSum FROM end " +
		"INNER JOIN round on round.RoundID = end.RoundID " +
		"INNER JOIN archer on archer.ArcherID = end.ArcherID " +
		"WHERE archer.ArcherID = 1 AND Approved = 1 " +
		"AND round.RoundName = 'Hobart' " +
		"AND Date between '2023-04-01' and '2023-04-20' " +
