...
type ArcherEndScoreROUNDNUM struct {
	EndID      int
	RoundNum   int
	FirstName  string
	LastName   string
...
type ArcherEndScoresRN []*ArcherEndScoreROUNDNUM

func (result ArcherEndScoresRN) Print(sql string) {
	println("query:", sql)
...
func GetEndCountOrderTotalByRoundNum(db *sql.DB, sqlStatement string) (QueryResult, error) {
	...
	var results ArcherEndScoresRN

	for rows.Next() {
		s := &ArcherEndScoreROUNDNUM{}
		if err := rows.Scan(
